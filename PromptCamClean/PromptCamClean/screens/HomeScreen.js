...
import { View, Text, ScrollView, TouchableOpacity, StyleSheet, SafeAreaView, StatusBar, Image, FlatList, Dimensions } from 'react-native';
...
function MiniCard({ item, navigation, isDark }) {
  ...
  return (
    <TouchableOpacity style={[styles.miniCard, { backgroundColor: isDark ? '#1e1e32' : '#fff' }]} onPress={() => navigation.navigate('TemplateDetail', { item })} activeOpacity={0.85}>
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setActiveIndex(prev => {
        ...
        try { flatRef.current?.scrollToIndex({ index: next, animated: true }); } catch (e) {}
        return next;
      });
    }, 4000);
    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: bg }]}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        <View style={styles.header}>
          ...
          <TouchableOpacity style={[styles.themeToggle, { backgroundColor: isDark ? '#2a2a4e' : '#f0f0f0' }]} onPress={toggleTheme} activeOpacity={0.8}>
        ...
        <View style={[styles.howBox, { backgroundColor: isDark ? '#1e1a08' : '#fff8ec', borderColor: isDark ? '#3a3010' : '#fde8b8' }]}>
          ...
          <Text style={[styles.howBoxText, { color: isDark ? '#f5c842' : '#c47f00' }]}>Tap any template → Copy prompt → Paste in AI editor → Get stunning results</Text>
        </View>
        ...
        <View style={styles.bannerWrap}>
          <FlatList ref={flatRef} data={trending} horizontal pagingEnabled showsHorizontalScrollIndicator={false}
            keyExtractor={item => String(item.id)} snapToInterval={CARD_WIDTH} decelerationRate="fast"
            onMomentumScrollEnd={e => { const idx = Math.round(e.nativeEvent.contentOffset.x / CARD_WIDTH); setActiveIndex(Math.max(0, Math.min(idx, trending.length - 1))); }}
            renderItem={({ item }) => (
              <TouchableOpacity style={styles.bannerCard} activeOpacity={0.92} onPress={() => navigation.navigate('TemplateDetail', { item })}>
                <Image source={{ uri: item.preview }} style={styles.bannerImage} resizeMode="cover" />
                <View style={styles.bannerOverlay} />
                <View style={styles.bannerBadge}><Text style={styles.bannerBadgeText}>{item.category.toUpperCase()}</Text></View>
                <View style={styles.bannerBottom}>
                  <Text style={styles.bannerEmoji}>{item.emoji}</Text>
                  <Text style={styles.bannerTitle}>{item.title}</Text>
                  <View style={styles.bannerCta}><Text style={styles.bannerCtaText}>Tap to view prompt →</Text></View>
                </View>
              </TouchableOpacity>
            )} />
          <View style={styles.dotsRow}>
            {trending.map((_, i) => (<View key={i} style={[styles.dot, i === activeIndex && styles.dotActive]} />))}
          </View>
        </View>
        <View style={styles.sectionHeader}>
        ...
        <View style={styles.listSection}>
          {popular.slice(0, 4).map(item => (<MiniCard key={item.id} item={item} navigation={navigation} isDark={isDark} />))}
        </View>
        <View style={styles.sectionHeader}>
...
const styles = StyleSheet.create({
  safe: { flex: 1 }, scroll: { paddingBottom: 20 },
  ...
  themeToggle: { width: 48, height: 48, borderRadius: 16, alignItems: 'center', justifyContent: 'center' },
  themeIcon: { fontSize: 22 },
  howBox: { flexDirection: 'row', alignItems: 'center', borderRadius: 14, marginHorizontal: 20, marginBottom: 24, paddingHorizontal: 14, paddingVertical: 12, borderWidth: 1 },
  howBoxIcon: { fontSize: 18, marginRight: 10 }, howBoxText: { fontSize: 12, fontWeight: '600', flex: 1 },
  sectionHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 20, marginBottom: 14 },
  sectionTitle: { fontSize: 20, fontWeight: '800' }, seeAll: { color: '#f5a623', fontSize: 14, fontWeight: '700' },
  ...
  bannerOverlay: { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.32)' },
  ...
  bannerEmoji: { fontSize: 22, marginBottom: 4 }, bannerTitle: { color: '#fff', fontSize: 20, fontWeight: '900', marginBottom: 10 },
  ...
  miniCard: { flexDirection: 'row', alignItems: 'center', borderRadius: 18, padding: 12, marginBottom: 12, shadowColor: '#000', shadowOffset: { width: 0, height: 3 }, shadowOpacity: 0.07, shadowRadius: 10, elevation: 3 },
  miniCardImage: { width: 64, height: 64, borderRadius: 14, backgroundColor: '#eee' },
  miniCardBody: { flex: 1, paddingHorizontal: 12 },
  miniCardTitle: { fontSize: 14, fontWeight: '700', marginBottom: 4 }, miniCardSub: { color: '#aaa', fontSize: 12, fontWeight: '500' },
  favBtn: { width: 36, height: 36, borderRadius: 12, backgroundColor: '#f4f4f4', alignItems: 'center', justifyContent: 'center', marginRight: 8 },
  favBtnActive: { backgroundColor: '#fff0f2' }, favBtnText: { fontSize: 17, color: '#ccc' }, favBtnTextActive: { color: '#ff4060' },
  arrowBtn: { width: 44, height: 44, borderRadius: 14, backgroundColor: '#f5a623', alignItems: 'center', justifyContent: 'center' },
