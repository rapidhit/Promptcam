      ...
      <View style={styles.layout}>
            ...
            {categories.map(cat => {
              ...
              return (
                <TouchableOpacity key={cat} style={[styles.sidebarItem, active && { backgroundColor: color + '18' }]} onPress={() => setActiveCategory(cat)} activeOpacity={0.75}>
                  <View style={[styles.sidebarIconWrap, { backgroundColor: active ? color : (isDark ? '#2a2a4e' : '#f4f4f4') }]}>
                    <Text style={styles.sidebarIcon}>{categoryIcons[cat] || '●'}</Text>
                  </View>
                  <Text style={[styles.sidebarLabel, { color: active ? color : (isDark ? '#666' : '#bbb'), fontWeight: active ? '800' : '600' }]} numberOfLines={2}>{cat}</Text>
        ...
        <View style={styles.main}>
          <View style={styles.mainHeader}>
            <Text style={[styles.mainTitle, { color: titleColor }]}>{categoryIcons[activeCategory]} {activeCategory}</Text>
...
const styles = StyleSheet.create({
  ...
  sidebarIconWrap: { width: 40, height: 40, borderRadius: 13, alignItems: 'center', justifyContent: 'center', marginBottom: 5 },
  sidebarIcon: { fontSize: 18 },
  sidebarLabel: { fontSize: 9, textAlign: 'center', lineHeight: 12 },
