    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: bg }]}>
      ...
      <View style={[styles.statsRow, { backgroundColor: cardBg }]}>
        <View style={styles.statBox}>
          <Text style={[styles.statBoxNum, { color: titleColor }]}>{favorited.length}</Text>
          <Text style={styles.statBoxLabel}>Favorites</Text>
        </View>
        <View style={styles.statDivider} />
        <View style={styles.statBox}>
          <Text style={[styles.statBoxNum, { color: titleColor }]}>{templates.length}</Text>
          <Text style={styles.statBoxLabel}>Total Prompts</Text>
        </View>
        <View style={styles.statDivider} />
        <View style={styles.statBox}>
          <Text style={[styles.statBoxNum, { color: titleColor }]}>{[...new Set(templates.map(t => t.category))].length}</Text>
      ...
      <FlatList
        ...
        ListHeaderComponent={
          favorited.length > 0
            ? <Text style={[styles.listHeader, { color: titleColor }]}>Saved Prompts</Text>
            : null
        }
        ListEmptyComponent={
          <View style={styles.emptyState}>
            ...
            <View style={styles.emptyHint}>
              <Text style={styles.emptyHintText}>Go to Explore or Categories → tap ♡</Text>
            </View>
          </View>
        }
        renderItem={({ item }) => (
...
const styles = StyleSheet.create({
  ...
  emptyHint: { backgroundColor: '#fff8ec', borderRadius: 12, paddingHorizontal: 16, paddingVertical: 10, borderWidth: 1, borderColor: '#fde8b8' },
  emptyHintText: { color: '#f5a623', fontSize: 12, fontWeight: '700' },
});
