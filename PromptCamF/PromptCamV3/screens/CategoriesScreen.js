...
const categoryIcons = { All: '✦', Lighting: '🌅', 'Color Grade': '🎬', Aesthetic: '🌸', Moody: '🖤', Film: '📷', Creative: '🎨', Seasonal: '❄️', Urban: '🌆', Clean: '☁️', Portrait: '✨' };
const categoryColors = { All: '#f5a623', Lighting: '#ff7f50', 'Color Grade': '#6c63ff', Aesthetic: '#ff69b4', Moody: '#4a4a6a', Film: '#8b6914', Creative: '#e74c3c', Seasonal: '#00bcd4', Urban: '#607d8b', Clean: '#26a69a', Portrait: '#ab47bc' };

export default function CategoriesScreen({ navigation }) {
  const [activeCategory, setActiveCategory] = useState('All');
  const { isFavorite, toggleFavorite } = useFavorites();
  const filtered = useMemo(() => activeCategory === 'All' ? templates : templates.filter(t => t.category === activeCategory), [activeCategory]);
        ...
        <View style={styles.sidebar}>
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            {categories.map(cat => {
              ...
              return (
                <TouchableOpacity key={cat} style={[styles.sidebarItem, active && { backgroundColor: color + '18' }]} onPress={() => setActiveCategory(cat)} activeOpacity={0.75}>
                  <View style={[styles.sidebarIconWrap, active && { backgroundColor: color }]}>
                    <Text style={styles.sidebarIcon}>{categoryIcons[cat] || '●'}</Text>
                  </View>
                  <Text style={[styles.sidebarLabel, active && { color, fontWeight: '800' }]} numberOfLines={2}>{cat}</Text>
        ...
        <View style={styles.main}>
          <View style={styles.mainHeader}>
            ...
            <View style={[styles.countPill, { backgroundColor: (categoryColors[activeCategory] || '#f5a623') + '20' }]}>
              <Text style={[styles.countText, { color: categoryColors[activeCategory] || '#f5a623' }]}>{filtered.length}</Text>
            </View>
          </View>
          <FlatList
            data={filtered}
...
const styles = StyleSheet.create({
  ...
  sidebar: { width: 82, backgroundColor: '#fff', borderRightWidth: 1, borderRightColor: '#efefef', paddingTop: 20, paddingHorizontal: 6, shadowColor: '#000', shadowOffset: { width: 3, height: 0 }, shadowOpacity: 0.05, shadowRadius: 8, elevation: 3 },
  sidebarTitle: { color: '#ccc', fontSize: 9, fontWeight: '800', letterSpacing: 2, textAlign: 'center', marginBottom: 14, textTransform: 'uppercase' },
  sidebarItem: { borderRadius: 14, padding: 8, alignItems: 'center', marginBottom: 4, position: 'relative' },
  sidebarIconWrap: { width: 40, height: 40, borderRadius: 13, backgroundColor: '#f4f4f4', alignItems: 'center', justifyContent: 'center', marginBottom: 5 },
  sidebarIcon: { fontSize: 18 },
  sidebarLabel: { color: '#bbb', fontSize: 9, fontWeight: '600', textAlign: 'center', lineHeight: 12 },
