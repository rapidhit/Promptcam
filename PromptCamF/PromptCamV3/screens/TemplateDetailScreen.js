...
import {
  ...
  ScrollView, Clipboard, Dimensions,
} from 'react-native';
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        ...
        {/* ── COPY BUTTON directly below image ── */}
        <View style={styles.copySection}>
          ...
          >
            ...
            <View>
              <Text style={styles.copyBtnTitle}>
                {copied ? 'Prompt Copied!' : 'Copy Prompt'}
              </Text>
              <Text style={styles.copyBtnSub}>
                {copied ? 'Now paste in your AI editor' : 'Tap to copy editing prompt'}
              </Text>
            </View>
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.howSection}>
          <Text style={styles.howTitle}>How to use this prompt</Text>
            ...
            {[
              ...
              <View key={step.num} style={styles.howStep}>
                ...
                <View style={styles.howStepBody}>
                  <Text style={styles.howStepLabel}>{step.icon}  {step.label}</Text>
        ...
        <View style={styles.promptSection}>
          <View style={styles.promptSectionHeader}>
            <Text style={styles.promptSectionTitle}>Full Prompt</Text>
            <TouchableOpacity onPress={() => setExpanded(!expanded)}>
              <Text style={styles.expandBtn}>{expanded ? 'Collapse ↑' : 'Expand ↓'}</Text>
            </TouchableOpacity>
          </View>
          <View style={styles.promptBox}>
            <Text style={styles.promptText} numberOfLines={expanded ? undefined : 4}>
              {item.prompt}
            </Text>
          </View>
        </View>

        {/* ── BOTTOM COPY BUTTON (sticky feel) ── */}
        <View style={styles.bottomCopyWrap}>
          <TouchableOpacity
            style={[styles.bottomCopyBtn, copied && styles.bottomCopyBtnSuccess]}
            onPress={handleCopy}
            activeOpacity={0.88}
          >
            <Text style={styles.bottomCopyText}>
...
const styles = StyleSheet.create({
  ...
  backBtnText: { fontSize: 20, color: '#1a1a2e', fontWeight: '700' },
  topBarTitle: {
    flex: 1, color: '#1a1a2e',
  ...
  // Copy section (right below image)
  copySection: {
  ...
  infoTitle: {
    ...
    fontWeight: '900', marginBottom: 12, letterSpacing: -0.3,
  },
  ...
  howSection: {
    backgroundColor: '#fff',
    marginTop: 10, paddingHorizontal: 20, paddingVertical: 20,
  },
  howTitle: {
  ...
  howStepLabel: { color: '#1a1a2e', fontSize: 14, fontWeight: '700', marginBottom: 2 },
  howStepSub: { color: '#aaa', fontSize: 12 },
  ...
  promptSectionTitle: { color: '#1a1a2e', fontSize: 17, fontWeight: '800' },
  expandBtn: { color: '#f5a623', fontSize: 13, fontWeight: '700' },
  promptBox: {
    backgroundColor: '#faf7f2',
    borderRadius: 16, padding: 16,
    borderWidth: 1.5, borderColor: '#f0e8d8',
